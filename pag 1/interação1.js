// MENU MOBILE
    ...
    function initCart() {
      ...
      function updateCart() {
  const stored = loadCart(); // carrega o que já existe no localStorage
  const existentes = stored.items || [];

  // Coleta os itens marcados apenas desta página
  const novos = Array.from(checkboxes)
    .filter(cb => cb.checked)
    .map(cb => ({ id: cb.id, label: cb.parentElement.textContent.trim(), value: cb.value }));

  // Remove do array existente todos os itens pertencentes a esta página atual
  const idsAtuais = Array.from(checkboxes).map(cb => cb.id);
  const filtrados = existentes.filter(item => !idsAtuais.includes(item.id));

  // Junta os antigos (de outras páginas) com os novos desta
  const merged = filtrados.concat(novos);

  const cart = { items: merged };
  saveCart(cart);
  renderCart(cart);
}
